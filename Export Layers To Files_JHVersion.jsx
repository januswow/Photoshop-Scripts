...
<javascriptresource>
<name>$$$/JavaScripts/ExportLayersToFilesJanusVer/Menu=Export Layers to Files...(Janus Ver.)</name>
...
var strCheckboxSerialFilename = localize("$$$/JavaScripts/ExportLayersToFiles/SerialFilename=&Add Serial Number to Filename");// Janus Huang
var strCheckboxGroupName = localize("$$$/JavaScripts/ExportLayersToFiles/GroupName=&Add Group Name to Filename");// Janus Huang
...
function settingDialog(exportInfo) {
	...
	// -- Janus Huang - add serial number to filename
    dlgMain.cbSerialFilename = dlgMain.grpTopLeft.add("checkbox", undefined, strCheckboxSerialFilename);
    dlgMain.cbSerialFilename.value = exportInfo.serialFilename;
	
	// -- Janus Huang - add group name to filename
    dlgMain.cbGroupName = dlgMain.grpTopLeft.add("checkbox", undefined, strCheckboxGroupName);
    dlgMain.cbGroupName.value = exportInfo.groupName;
	...
	exportInfo.serialFilename = dlgMain.cbSerialFilename.value;// Janus Huang
	exportInfo.groupName = dlgMain.cbGroupName.value;// Janus Huang
...
function initExportInfo(exportInfo) {
	...
	exportInfo.serialFilename = false;//Janus Huang
	exportInfo.groupName = false;//Janus Huang
...
function exportChildren(dupObj, orgObj, exportInfo, dupDocRef, fileNamePrefix) {
    for( var i = 0; i < dupObj.artLayers.length; i++) {
		...
		// Janus Huang --Mod Start
		// Get new filename
		var fileNameBody = ""
		if (fileNamePrefix.length>0) {
			fileNameBody += fileNamePrefix
			fileNameBody = AddUnderscore(fileNameBody);
		}
		if (exportInfo.serialFilename) fileNameBody += zeroSuppress(i, 4) + "_";
    ...
    for( var i = 0; i < dupObj.layerSets.length; i++) {
		...
		// Get new filename
		var fileNameBody = ""
		if (fileNamePrefix.length>0) fileNameBody += fileNamePrefix + "_";
		if (exportInfo.serialFilename) fileNameBody += zeroSuppress(i, 4) + "s_";
		if (exportInfo.groupName) fileNameBody += dupObj.layerSets[i].name;
		fileNameBody = AddUnderscore(fileNameBody);
		// Janus Huang --Mod End		
...
// Usage: add underscore to string // Janus Huang
// Input: string
// Return: string
///////////////////////////////////////////////////////////////////////////
function AddUnderscore( s ) {
	if (s.substring(s.length-1, s.length) != "_") s += "_";
	return s;
}
